-- ============================================================================
-- BIGQUERY + BQML: 1-WEEK AHEAD STOCKOUT FORECAST (HORIZON h=1)
-- ============================================================================
-- Purpose: Production-ready implementation of 3-layer probabilistic forecasting
--   Layer 1: OOS Classification → p_oos_h1 (stockout risk at t+1)
--   Layer 2: Demand Regression → yhat_p50_h1 (median demand at t+1)
--   Layer 3: Empirical Quantiles → q90_h1, q95_h1 (stratified by 20 segments)
--
-- Architecture:
--   - Strict leakage control (no future information in features)
--   - ISO week arithmetic (week_start_date + 1 week)
...
--
-- OUTPUT ARTIFACTS:
--   - Models: m_oos_h1, m_demand_h1
--   - Tables: weekly_features_h1, forecast_h1, alerts_top100_h1
--   - Evaluation: eval_alerts_h1, eval_coverage_h1, leakage_check_h1
--
...
DECLARE HORIZON_WEEKS INT64 DEFAULT 1;  -- Forecast horizon (t+1 week)
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.vip_customers_h1` AS
...
shares AS (
  ...
  LEFT JOIN `{PROJECT_ID}.{BQ_DATASET}.vip_customers_h1` v
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.weekly_features_h1` AS
WITH base AS (
  SELECT
    b.sku_id,
    b.week_start_date,
...
final AS (
  SELECT
    t.*,
    DATE_ADD(t.week_start_date, INTERVAL HORIZON_WEEKS WEEK) AS label_week_h1
...
labeled AS (
  SELECT
    ...
    t.stockout_event,
    t.label_week_h1,
    t1.y_sales AS y_true_h1,
    t1.stockout_event AS stockout_event_h1,
  ...
  LEFT JOIN final t1
   ...
   AND t1.week_start_date = t.label_week_h1
)
SELECT *
FROM labeled
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.train_calib_split_h1` AS
SELECT
  *,
  CASE
...
FROM `{PROJECT_ID}.{BQ_DATASET}.weekly_features_h1`
WHERE y_true_h1 IS NOT NULL
  AND label_week_h1 <= VAL_END_DATE;

-- ----------------------------------------------------------------------------
-- SECTION 3B: DIAGNOSTICS — WHALES & PRICE QUALITY
-- ----------------------------------------------------------------------------
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.diag_whales_price_h1` AS
SELECT
  split,
  COUNT(*) AS n_rows,
...
FROM `{PROJECT_ID}.{BQ_DATASET}.train_calib_split_h1`
GROUP BY split
ORDER BY split;
...
CREATE OR REPLACE MODEL `{PROJECT_ID}.{BQ_DATASET}.m_oos_h1`
OPTIONS(
  MODEL_TYPE='BOOSTED_TREE_CLASSIFIER',
  INPUT_LABEL_COLS=['stockout_event_h1'],
...
SELECT
  CAST(stockout_event_h1 AS INT64) AS stockout_event_h1,
  is_train,
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.eval_classifier_h1` AS
SELECT
  'OOS_Classifier_h1' AS model_name,
  *
FROM ML.EVALUATE(
  MODEL `{PROJECT_ID}.{BQ_DATASET}.m_oos_h1`,
  (SELECT * FROM `{PROJECT_ID}.{BQ_DATASET}.train_calib_split_h1` WHERE split = 'VAL')
);
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.score_oos_h1_all` AS
SELECT
  t.sku_id,
  t.week_start_date,
  t.split,
  CAST(t.stockout_event_h1 AS INT64) AS y_true,
  COALESCE(
    (SELECT prob FROM UNNEST(p.predicted_stockout_event_h1_probs) WHERE SAFE_CAST(label AS INT64) = 0),
    (SELECT prob FROM UNNEST(p.predicted_stockout_event_h1_probs) WHERE LOWER(CAST(label AS STRING)) IN ('0', 'false', 'no')),
    NULL
  ) AS p0_raw,
  COALESCE(
    (SELECT prob FROM UNNEST(p.predicted_stockout_event_h1_probs) WHERE SAFE_CAST(label AS INT64) = 1),
    (SELECT prob FROM UNNEST(p.predicted_stockout_event_h1_probs) WHERE LOWER(CAST(label AS STRING)) IN ('1', 'true', 'yes')),
    NULL
  ) AS p1_raw,
  p.predicted_stockout_event_h1 AS predicted_label
FROM `{PROJECT_ID}.{BQ_DATASET}.train_calib_split_h1` t
LEFT JOIN ML.PREDICT(
  MODEL `{PROJECT_ID}.{BQ_DATASET}.m_oos_h1`,
  (SELECT * FROM `{PROJECT_ID}.{BQ_DATASET}.train_calib_split_h1`)
) p
USING (sku_id, week_start_date);

CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.label_values_oos_h1` AS
SELECT DISTINCT
  label AS observed_label,
  COUNT(*) OVER (PARTITION BY label) AS n_occurrences
FROM `{PROJECT_ID}.{BQ_DATASET}.score_oos_h1_all`
CROSS JOIN UNNEST(
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.oos_prob_mapping_h1` AS
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.score_oos_h1_final` AS
SELECT
  s.*,
  CASE 
    WHEN (SELECT pos_label FROM `{PROJECT_ID}.{BQ_DATASET}.oos_prob_mapping_h1`) = '1' 
    THEN s.p1_raw 
    ELSE s.p0_raw 
  END AS p_oos_h1,
  CASE 
    ...
    ELSE s.p1_raw 
  END AS p_non_oos_h1
FROM `{PROJECT_ID}.{BQ_DATASET}.score_oos_h1_all` s;

-- ============================================================================
-- SECTION 4B: PROBABILITY CALIBRATION (PLATT SCALING ON CALIB)
-- ============================================================================

CREATE OR REPLACE MODEL `{PROJECT_ID}.{BQ_DATASET}.m_platt_oos_h1`
OPTIONS(
...
SELECT
  ...
  CAST(p_oos_h1 AS FLOAT64) AS p_oos_raw
FROM `{PROJECT_ID}.{BQ_DATASET}.score_oos_h1_final`
WHERE split = 'CALIB'
  AND p_oos_h1 IS NOT NULL;

CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.score_oos_h1_calibrated` AS
SELECT
  s.*,
  COALESCE(
...
FROM `{PROJECT_ID}.{BQ_DATASET}.score_oos_h1_final` s
LEFT JOIN ML.PREDICT(
  MODEL `{PROJECT_ID}.{BQ_DATASET}.m_platt_oos_h1`,
  (SELECT sku_id, week_start_date, CAST(p_oos_h1 AS FLOAT64) AS p_oos_raw
   FROM `{PROJECT_ID}.{BQ_DATASET}.score_oos_h1_final`
   WHERE p_oos_h1 IS NOT NULL)
) p
USING (sku_id, week_start_date);

CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.eval_platt_oos_h1` AS
SELECT
  split,
  COUNT(*) AS n_obs,
  SUM(y_true) AS n_pos,
  AVG(y_true) AS prevalence,
  AVG(p_oos_h1) AS mean_p_raw,
  AVG(p_oos_h1_cal) AS mean_p_cal,
  AVG(POW(p_oos_h1 - y_true, 2)) AS brier_raw,
  AVG(POW(p_oos_h1_cal - y_true, 2)) AS brier_cal
FROM `{PROJECT_ID}.{BQ_DATASET}.score_oos_h1_calibrated`
WHERE p_oos_h1 IS NOT NULL AND p_oos_h1_cal IS NOT NULL
GROUP BY split
ORDER BY split;

CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.calib_deciles_oos_h1_val_platt` AS
WITH d AS (
  SELECT
    NTILE(10) OVER (ORDER BY p_oos_h1) AS decile_raw,
    NTILE(10) OVER (ORDER BY p_oos_h1_cal) AS decile_cal,
    y_true,
    p_oos_h1,
    p_oos_h1_cal
  FROM `{PROJECT_ID}.{BQ_DATASET}.score_oos_h1_calibrated`
  WHERE split = 'VAL'
    AND p_oos_h1 IS NOT NULL
    AND p_oos_h1_cal IS NOT NULL
)
SELECT
  'RAW' AS version,
  decile_raw AS decile,
  COUNT(*) AS n_obs,
  AVG(p_oos_h1) AS mean_pred,
  AVG(y_true) AS obs_rate,
  ABS(AVG(p_oos_h1) - AVG(y_true)) AS calib_error
FROM d
GROUP BY decile
UNION ALL
SELECT
  ...
  COUNT(*) AS n_obs,
  AVG(p_oos_h1_cal) AS mean_pred,
  AVG(y_true) AS obs_rate,
  ABS(AVG(p_oos_h1_cal) - AVG(y_true)) AS calib_error
FROM d
GROUP BY decile
ORDER BY version, decile;

CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.diag_oos_h1_by_split` AS
SELECT
  split,
  COUNT(*) AS n_total,
  SUM(y_true) AS n_positive,
  AVG(y_true) AS prevalence,
  AVG(p_oos_h1) AS mean_p_oos,
  STDDEV(p_oos_h1) AS stddev_p_oos,
  MIN(p_oos_h1) AS min_p_oos,
  MAX(p_oos_h1) AS max_p_oos,
  AVG(CASE WHEN y_true = 1 THEN p_oos_h1 END) AS mean_p_oos_given_pos,
  AVG(CASE WHEN y_true = 0 THEN p_oos_h1 END) AS mean_p_oos_given_neg,
  AVG(CASE WHEN y_true = 1 THEN p_oos_h1 END) - AVG(CASE WHEN y_true = 0 THEN p_oos_h1 END) AS signal_separation,
  AVG(CASE WHEN p_oos_h1 IS NULL THEN 1.0 ELSE 0.0 END) AS null_rate_p_oos,
  AVG(p_oos_h1 + p_non_oos_h1) AS mean_sum_probs,
  STDDEV(p_oos_h1 + p_non_oos_h1) AS stddev_sum_probs,
  CASE WHEN AVG(CASE WHEN y_true = 1 THEN p_oos_h1 END) > AVG(CASE WHEN y_true = 0 THEN p_oos_h1 END) 
       THEN 'PASS' ELSE 'FAIL' END AS signal_direction_check,
  CASE WHEN STDDEV(p_oos_h1) > 0.01 THEN 'PASS' ELSE 'FAIL' END AS variance_check
FROM `{PROJECT_ID}.{BQ_DATASET}.score_oos_h1_final`
GROUP BY split
ORDER BY split;

CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.calib_deciles_oos_h1_traincalib` AS
WITH deciles AS (
  SELECT
    NTILE(10) OVER (ORDER BY p_oos_h1) AS decile,
    p_oos_h1,
    y_true
  FROM `{PROJECT_ID}.{BQ_DATASET}.score_oos_h1_final`
  WHERE split IN ('TRAIN', 'CALIB')
...
SELECT
  decile,
  COUNT(*) AS n_obs,
  AVG(p_oos_h1) AS mean_predicted_prob,
  MIN(p_oos_h1) AS min_p_oos,
  MAX(p_oos_h1) AS max_p_oos,
  AVG(y_true) AS observed_rate,
  ABS(AVG(p_oos_h1) - AVG(y_true)) AS calibration_error,
  CASE WHEN ABS(AVG(p_oos_h1) - AVG(y_true)) < 0.05 THEN 'PASS' ELSE 'FAIL' END AS calib_check
FROM deciles
GROUP BY decile
ORDER BY decile;
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.train_with_poos_h1` AS
SELECT
  t.*,
  COALESCE(s.p_oos_h1_cal, s.p_oos_h1, 0.5) AS p_oos_h1
FROM `{PROJECT_ID}.{BQ_DATASET}.train_calib_split_h1` t
LEFT JOIN `{PROJECT_ID}.{BQ_DATASET}.score_oos_h1_calibrated` s
USING (sku_id, week_start_date);

CREATE OR REPLACE MODEL `{PROJECT_ID}.{BQ_DATASET}.m_demand_h1`
OPTIONS(
  MODEL_TYPE='BOOSTED_TREE_REGRESSOR',
  INPUT_LABEL_COLS=['y_true_h1'],
  DATA_SPLIT_METHOD='CUSTOM',
  DATA_SPLIT_COL='is_train',
  MAX_ITERATIONS=100,
  LEARN_RATE=0.05,
...
SELECT
  ...
  weeks_since_start,
  p_oos_h1
FROM `{PROJECT_ID}.{BQ_DATASET}.train_with_poos_h1`
WHERE split IN ('TRAIN', 'CALIB');

CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.eval_regressor_h1` AS
SELECT
  'Demand_Regressor_h1' AS model_name,
  *
FROM ML.EVALUATE(
  MODEL `{PROJECT_ID}.{BQ_DATASET}.m_demand_h1`,
  (SELECT * FROM `{PROJECT_ID}.{BQ_DATASET}.train_with_poos_h1` WHERE split = 'VAL')
);
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.residuals_h1` AS
WITH predictions AS (
  SELECT
    t.sku_id,
    t.week_start_date,
    ...
    t.y_true_h1,
    t.amplitude,
    p.predicted_y_true_h1 AS yhat_p50_h1
  FROM `{PROJECT_ID}.{BQ_DATASET}.train_with_poos_h1` t
  LEFT JOIN ML.PREDICT(
    MODEL `{PROJECT_ID}.{BQ_DATASET}.m_demand_h1`,
    (SELECT * FROM `{PROJECT_ID}.{BQ_DATASET}.train_with_poos_h1` WHERE split = 'CALIB')
  ) p
  USING (sku_id, week_start_date)
  WHERE t.split = 'CALIB'
)
SELECT
  ...
  y_true_h1,
  yhat_p50_h1,
  y_true_h1 - yhat_p50_h1 AS residual,
  amplitude
FROM predictions;
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.quantile_lookup_h1` AS
WITH segment_stats AS (
  ...
  FROM `{PROJECT_ID}.{BQ_DATASET}.residuals_h1`
...
-- ----------------------------------------------------------------------------
-- SECTION 8: FORECAST TABLE (POINT + QUANTILES FOR h=1)
-- ----------------------------------------------------------------------------

CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.forecast_h1` AS
WITH base_predictions AS (
  SELECT
    ...
    t.amplitude,
    COALESCE(s.p_oos_h1_cal, s.p_oos_h1, 0.5) AS p_oos_h1,
    p_demand.predicted_y_true_h1 AS yhat_p50_h1
  FROM `{PROJECT_ID}.{BQ_DATASET}.train_with_poos_h1` t
  LEFT JOIN `{PROJECT_ID}.{BQ_DATASET}.score_oos_h1_calibrated` s
...
SELECT
  ...
  f.label_week_h1 AS target_week,
  f.iso_week,
  f.season_group,
  f.demand_decile,
  f.segment_id,
  ...
  f.p_oos_h1,
  f.yhat_p50_h1,
  f.yhat_p50_h1 + q.q_resid_p90 AS q90_h1,
  f.yhat_p50_h1 + q.q_resid_p95 AS q95_h1,
  f.yhat_p50_h1 + q.q_resid_p99 AS q99_h1,
  f.amplitude,
  q.n_obs AS segment_n_calib,
  q.std_residual AS segment_std_resid
FROM base_predictions f
LEFT JOIN `{PROJECT_ID}.{BQ_DATASET}.quantile_lookup_h1` q
USING (segment_id)
ORDER BY decision_week, p_oos_h1 DESC;

-- ----------------------------------------------------------------------------
-- SECTION 9: ALERT RANKING (TOP 100 PER WEEK)
-- ----------------------------------------------------------------------------

CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.alerts_top100_h1` AS
WITH base AS (
  SELECT
    f.decision_week,
    f.target_week,
    ...
    f.segment_id,
    f.amplitude
  FROM `{PROJECT_ID}.{BQ_DATASET}.forecast_h1` f
),
labels AS (
  SELECT
    sku_id,
    label_week_h1 AS target_week,
    CAST(stockout_event_h1 AS INT64) AS stockout_event_h1
  FROM `{PROJECT_ID}.{BQ_DATASET}.train_calib_split_h1`
  WHERE label_week_h1 IS NOT NULL
),
enriched AS (
  SELECT
    b.*,
    COALESCE(l.stockout_event_h1, NULL) AS true_stockout_label_model,
    CASE WHEN b.y_true_h1 = 0 THEN 1 ELSE 0 END AS true_stockout_label_sales0,
    GREATEST(0, b.q95_h1 - b.yhat_p50_h1) AS uncertainty_width_p95,
    (b.p_oos_h1 * GREATEST(0, b.q95_h1 - b.yhat_p50_h1)) AS risk_score
  FROM base b
  LEFT JOIN labels l
  ...
  SELECT
    *,
    ROW_NUMBER() OVER (
      ...
      ORDER BY risk_score DESC, p_oos_h1 DESC, amplitude DESC
    ) AS rank_in_week
  FROM enriched
)
SELECT
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.eval_alerts_top100_h1` AS
WITH alert_stats AS (
  SELECT
    decision_week,
    season_group,
    ...
    AVG(p_oos_h1) AS mean_p_oos_in_top100,
    MIN(p_oos_h1) AS min_p_oos_in_top100,
    AVG(risk_score) AS mean_risk_score_in_top100
  FROM `{PROJECT_ID}.{BQ_DATASET}.alerts_top100_h1`
...
total_stockouts AS (
  SELECT
    f.decision_week,
    f.season_group,
    SUM(CAST(stockout_event_h1 AS INT64)) AS n_total_stockouts_model,
    AVG(CAST(stockout_event_h1 AS INT64)) AS prevalence_model,
    SUM(CASE WHEN f.y_true_h1 = 0 THEN 1 ELSE 0 END) AS n_total_stockouts_sales0,
    AVG(CASE WHEN f.y_true_h1 = 0 THEN 1.0 ELSE 0.0 END) AS prevalence_sales0
  FROM `{PROJECT_ID}.{BQ_DATASET}.forecast_h1` f
  LEFT JOIN `{PROJECT_ID}.{BQ_DATASET}.train_calib_split_h1` t
    ON f.sku_id = t.sku_id AND f.target_week = t.label_week_h1
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.eval_alerts_top100_h1_pooled` AS
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.eval_alerts_h1` AS
WITH alert_stats AS (
  SELECT
    decision_week,
    season_group,
...
total_stockouts AS (
  SELECT
    f.decision_week,
    f.season_group,
    SUM(CAST(stockout_event_h1 AS INT64)) AS n_total_stockouts
  FROM `{PROJECT_ID}.{BQ_DATASET}.forecast_h1` f
  LEFT JOIN `{PROJECT_ID}.{BQ_DATASET}.train_calib_split_h1` t
    ON f.sku_id = t.sku_id AND f.target_week = t.label_week_h1
  GROUP BY f.decision_week, f.season_group
...
FROM `{PROJECT_ID}.{BQ_DATASET}.eval_alerts_h1`
GROUP BY season_group
ORDER BY season_group;
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.eval_coverage_h1` AS
WITH violations AS (
  SELECT
    season_group,
    segment_id,
    COUNT(*) AS n_obs,
    SUM(CASE WHEN y_true_h1 > q90_h1 THEN 1 ELSE 0 END) AS n_violations_p90,
    SUM(CASE WHEN y_true_h1 > q95_h1 THEN 1 ELSE 0 END) AS n_violations_p95,
    SUM(CASE WHEN y_true_h1 > q99_h1 THEN 1 ELSE 0 END) AS n_violations_p99,
    AVG(CASE WHEN y_true_h1 > q90_h1 THEN 1.0 ELSE 0.0 END) AS viol_rate_p90,
    AVG(CASE WHEN y_true_h1 > q95_h1 THEN 1.0 ELSE 0.0 END) AS viol_rate_p95,
    AVG(CASE WHEN y_true_h1 > q99_h1 THEN 1.0 ELSE 0.0 END) AS viol_rate_p99,
    0.10 AS nominal_rate_p90,
    0.05 AS nominal_rate_p95,
    0.01 AS nominal_rate_p99
  FROM `{PROJECT_ID}.{BQ_DATASET}.forecast_h1`
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.eval_coverage_h1_conditional` AS
WITH base AS (
  SELECT *
  FROM `{PROJECT_ID}.{BQ_DATASET}.forecast_h1`
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.eval_coverage_summary_h1_conditional` AS
SELECT
  season_group,
...
FROM `{PROJECT_ID}.{BQ_DATASET}.eval_coverage_h1_conditional`
GROUP BY season_group
ORDER BY season_group;
...
FROM `{PROJECT_ID}.{BQ_DATASET}.eval_coverage_h1`
GROUP BY season_group
ORDER BY season_group;
...
CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.leakage_check_h1` AS
SELECT
  'Leakage Check h=1' AS test_name,
  COUNT(*) AS n_rows_checked,
  SUM(CASE WHEN DATE_DIFF(label_week_h1, week_start_date, WEEK) <> 1 THEN 1 ELSE 0 END) AS n_wrong_horizon,
  SUM(CASE WHEN lag_1 IS NULL AND week_start_date > '2020-01-13' THEN 1 ELSE 0 END) AS n_missing_lag1,
  CASE 
    WHEN SUM(CASE WHEN DATE_DIFF(label_week_h1, week_start_date, WEEK) <> 1 THEN 1 ELSE 0 END) = 0 
...
SELECT * FROM `{PROJECT_ID}.{BQ_DATASET}.leakage_check_h1`;

-- ----------------------------------------------------------------------------
-- SECTION 13: FINAL SUMMARY REPORT
-- ----------------------------------------------------------------------------

CREATE OR REPLACE TABLE `{PROJECT_ID}.{BQ_DATASET}.run_summary_h1` AS
SELECT
  CURRENT_TIMESTAMP() AS run_ts,
  'h1' AS horizon,
  (SELECT roc_auc FROM `{PROJECT_ID}.{BQ_DATASET}.eval_classifier_h1`) AS oos_auc_val,
  (SELECT recall FROM `{PROJECT_ID}.{BQ_DATASET}.eval_classifier_h1`) AS oos_recall_val,
  (SELECT precision FROM `{PROJECT_ID}.{BQ_DATASET}.eval_classifier_h1`) AS oos_precision_val,
  (SELECT log_loss FROM `{PROJECT_ID}.{BQ_DATASET}.eval_classifier_h1`) AS oos_logloss_val,
  (SELECT mean_absolute_error FROM `{PROJECT_ID}.{BQ_DATASET}.eval_regressor_h1`) AS demand_mae_val,
  (SELECT r2_score FROM `{PROJECT_ID}.{BQ_DATASET}.eval_regressor_h1`) AS demand_r2_val,
  (SELECT brier_raw FROM `{PROJECT_ID}.{BQ_DATASET}.eval_platt_oos_h1` WHERE split='VAL') AS brier_raw_val,
  (SELECT brier_cal FROM `{PROJECT_ID}.{BQ_DATASET}.eval_platt_oos_h1` WHERE split='VAL') AS brier_cal_val,
  (SELECT mean_p_raw FROM `{PROJECT_ID}.{BQ_DATASET}.eval_platt_oos_h1` WHERE split='VAL') AS mean_p_raw_val,
  (SELECT mean_p_cal FROM `{PROJECT_ID}.{BQ_DATASET}.eval_platt_oos_h1` WHERE split='VAL') AS mean_p_cal_val,
  (SELECT prevalence FROM `{PROJECT_ID}.{BQ_DATASET}.eval_platt_oos_h1` WHERE split='VAL') AS prevalence_val,
  (SELECT precision_at_100_model FROM `{PROJECT_ID}.{BQ_DATASET}.eval_alerts_top100_h1_pooled` WHERE season_group='HIGH_SEASON') AS prec100_model_high,
  (SELECT recall_at_100_model FROM `{PROJECT_ID}.{BQ_DATASET}.eval_alerts_top100_h1_pooled` WHERE season_group='HIGH_SEASON') AS rec100_model_high,
  (SELECT lift_at_100_model FROM `{PROJECT_ID}.{BQ_DATASET}.eval_alerts_top100_h1_pooled` WHERE season_group='HIGH_SEASON') AS lift100_model_high,
  (SELECT precision_at_100_model FROM `{PROJECT_ID}.{BQ_DATASET}.eval_alerts_top100_h1_pooled` WHERE season_group='REST') AS prec100_model_rest,
  (SELECT recall_at_100_model FROM `{PROJECT_ID}.{BQ_DATASET}.eval_alerts_top100_h1_pooled` WHERE season_group='REST') AS rec100_model_rest,
  (SELECT lift_at_100_model FROM `{PROJECT_ID}.{BQ_DATASET}.eval_alerts_top100_h1_pooled` WHERE season_group='REST') AS lift100_model_rest,
  (SELECT SAFE_DIVIDE(SUM(n_violations_p90), SUM(n_obs)) FROM `{PROJECT_ID}.{BQ_DATASET}.eval_coverage_h1` WHERE season_group='HIGH_SEASON') AS viol_p90_uncond_high,
  (SELECT SAFE_DIVIDE(SUM(n_violations_p95), SUM(n_obs)) FROM `{PROJECT_ID}.{BQ_DATASET}.eval_coverage_h1` WHERE season_group='HIGH_SEASON') AS viol_p95_uncond_high,
  (SELECT SAFE_DIVIDE(SUM(n_violations_p90), SUM(n_obs)) FROM `{PROJECT_ID}.{BQ_DATASET}.eval_coverage_h1` WHERE season_group='REST') AS viol_p90_uncond_rest,
  (SELECT SAFE_DIVIDE(SUM(n_violations_p95), SUM(n_obs)) FROM `{PROJECT_ID}.{BQ_DATASET}.eval_coverage_h1` WHERE season_group='REST') AS viol_p95_uncond_rest,
  (SELECT viol_rate_p90 FROM `{PROJECT_ID}.{BQ_DATASET}.eval_coverage_summary_h1_conditional` WHERE season_group='HIGH_SEASON') AS viol_p90_cond_high,
  (SELECT viol_rate_p95 FROM `{PROJECT_ID}.{BQ_DATASET}.eval_coverage_summary_h1_conditional` WHERE season_group='HIGH_SEASON') AS viol_p95_cond_high,
  (SELECT viol_rate_p90 FROM `{PROJECT_ID}.{BQ_DATASET}.eval_coverage_summary_h1_conditional` WHERE season_group='REST') AS viol_p90_cond_rest,
  (SELECT viol_rate_p95 FROM `{PROJECT_ID}.{BQ_DATASET}.eval_coverage_summary_h1_conditional` WHERE season_group='REST') AS viol_p95_cond_rest,
  DEMAND_ACTIVE_THRESHOLD AS demand_active_threshold
;

SELECT '============================================================' AS sep;
SELECT 'BIGQUERY STOCKOUT FORECAST h=1 — EXECUTION COMPLETE' AS title;
SELECT '============================================================' AS sep;
