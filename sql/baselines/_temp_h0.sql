...
CREATE OR REPLACE TABLE `thequantitativeledger.cruzber_models_eu.score_h0_heuristic_val` AS
...
CREATE OR REPLACE TABLE `thequantitativeledger.cruzber_models_eu.eval_h0_heuristic` AS
WITH predictions AS (
  SELECT
    y_true,
    score_h0_heuristic AS predicted_score,
    CASE WHEN score_h0_heuristic >= 0.5 THEN 1 ELSE 0 END AS predicted_label
  FROM `thequantitativeledger.cruzber_models_eu.score_h0_heuristic_val`
),
confusion AS (
  SELECT
...
CREATE OR REPLACE TABLE `thequantitativeledger.cruzber_models_eu.auc_h0_heuristic` AS
...
SELECT 
  ...
  roc_auc,
  (SELECT precision FROM `thequantitativeledger.cruzber_models_eu.eval_h0_heuristic`) AS precision,
  (SELECT recall FROM `thequantitativeledger.cruzber_models_eu.eval_h0_heuristic`) AS recall,
  (SELECT f1_score FROM `thequantitativeledger.cruzber_models_eu.eval_h0_heuristic`) AS f1_score,
  (SELECT accuracy FROM `thequantitativeledger.cruzber_models_eu.eval_h0_heuristic`) AS accuracy
FROM auc_final;

-- Step 4: Precision@K and Lift@K (top 100, 500, 1000)
CREATE OR REPLACE TABLE `thequantitativeledger.cruzber_models_eu.precision_at_k_h0` AS
...
precision_at_k AS (
  SELECT
    ...
    (SELECT AVG(y_true) FROM `thequantitativeledger.cruzber_models_eu.score_h0_heuristic_val`) AS baseline_precision,
    
    -- Lift
    SAFE_DIVIDE(
      SAFE_DIVIDE(COUNTIF(rp.y_true = 1), k),
      (SELECT AVG(y_true) FROM `thequantitativeledger.cruzber_models_eu.score_h0_heuristic_val`)
    ) AS lift_at_k
  FROM k_values kv
...
SELECT 
  ...
  f1_score,
  CASE
    WHEN roc_auc >= 0.70 THEN 'âœ… Reasonable baseline'
    WHEN roc_auc >= 0.60 THEN 'âš ï¸ Weak baseline'
