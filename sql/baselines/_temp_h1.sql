...
CREATE OR REPLACE MODEL `thequantitativeledger.cruzber_models_eu.m_baseline_h1_logistic`
OPTIONS (
...
CREATE OR REPLACE TABLE `thequantitativeledger.cruzber_models_eu.score_h1_logistic_val` AS
SELECT
  sku_id,
  week_start_date,
  y_oos_h4 AS y_true,
  ...
  split
FROM ML.PREDICT(
  MODEL `thequantitativeledger.cruzber_models_eu.m_baseline_h1_logistic`,
  (SELECT * FROM `thequantitativeledger.cruzber_models_eu.weekly_features_h4` WHERE split = 'VAL')
);

-- Step 3: Evaluate model (standard BQML metrics)
CREATE OR REPLACE TABLE `thequantitativeledger.cruzber_models_eu.eval_h1_logistic` AS
SELECT 
...
CREATE OR REPLACE TABLE `thequantitativeledger.cruzber_models_eu.precision_at_k_h1` AS
WITH ranked_preds AS (
  SELECT
    y_true,
    score_h1_logistic,
    ROW_NUMBER() OVER (ORDER BY score_h1_logistic DESC) AS rank
  FROM `thequantitativeledger.cruzber_models_eu.score_h1_logistic_val`
),
k_values AS (
...
precision_at_k AS (
  SELECT
    ...
    (SELECT AVG(y_true) FROM `thequantitativeledger.cruzber_models_eu.score_h1_logistic_val`) AS baseline_precision,
    SAFE_DIVIDE(
      SAFE_DIVIDE(COUNTIF(rp.y_true = 1), k),
      (SELECT AVG(y_true) FROM `thequantitativeledger.cruzber_models_eu.score_h1_logistic_val`)
    ) AS lift_at_k
  FROM k_values kv
...
CREATE OR REPLACE TABLE `thequantitativeledger.cruzber_models_eu.feature_importance_h1` AS
SELECT
  'H1_Logistic' AS model_name,
  feature,
...
FROM ML.WEIGHTS(MODEL `thequantitativeledger.cruzber_models_eu.m_baseline_h1_logistic`)
...
SELECT
  ...
  log_loss,
  CASE
    WHEN roc_auc >= 0.85 THEN 'âœ… Strong linear baseline'
    WHEN roc_auc >= 0.70 THEN 'âœ… Reasonable linear baseline'
    WHEN roc_auc >= 0.60 THEN 'âš ï¸ Weak linear baseline'
