...
--
-- Logic:
--   - If SKU was OOS in previous period → likely OOS now
...
CREATE OR REPLACE TABLE `{dataset_ref}.features_temporal_h2` AS
...
CREATE OR REPLACE TABLE `{dataset_ref}.score_h2_temporal_val` AS
SELECT
  sku_id,
  week_start_date,
  y_oos_h4 AS y_true,
  ...
  prev_oos_proxy,
  split
FROM `{dataset_ref}.features_temporal_h2`
WHERE split = 'VAL';

-- Step 3: Calculate metrics
CREATE OR REPLACE TABLE `{dataset_ref}.eval_h2_temporal` AS
WITH predictions AS (
  SELECT
    y_true,
    score_h2_temporal AS predicted_score,
    predicted_label
  FROM `{dataset_ref}.score_h2_temporal_val`
),
confusion AS (
  SELECT
...
CREATE OR REPLACE TABLE `{dataset_ref}.auc_h2_temporal` AS
...
SELECT 
  ...
  roc_auc,
  (SELECT precision FROM `{dataset_ref}.eval_h2_temporal`) AS precision,
  (SELECT recall FROM `{dataset_ref}.eval_h2_temporal`) AS recall,
  (SELECT f1_score FROM `{dataset_ref}.eval_h2_temporal`) AS f1_score,
  (SELECT accuracy FROM `{dataset_ref}.eval_h2_temporal`) AS accuracy
FROM auc_final;

-- Step 5: Precision@K and Lift@K
CREATE OR REPLACE TABLE `{dataset_ref}.precision_at_k_h2` AS
...
precision_at_k AS (
  SELECT
    ...
    (SELECT AVG(y_true) FROM `{dataset_ref}.score_h2_temporal_val`) AS baseline_precision,
    SAFE_DIVIDE(
      SAFE_DIVIDE(COUNTIF(rp.y_true = 1), k),
      (SELECT AVG(y_true) FROM `{dataset_ref}.score_h2_temporal_val`)
    ) AS lift_at_k
  FROM k_values kv
...
SELECT 
  ...
  f1_score,
  CASE
    WHEN roc_auc >= 0.75 THEN '✅ Strong temporal baseline'
    WHEN roc_auc >= 0.65 THEN '✅ Reasonable temporal baseline'
    WHEN roc_auc >= 0.55 THEN '⚠️ Weak temporal baseline'
    ELSE '❌ Poor baseline'
  END AS verdict
