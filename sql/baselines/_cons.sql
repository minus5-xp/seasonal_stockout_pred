...
CREATE OR REPLACE TABLE `thequantitativeledger.cruzber_models_eu.baselines_comparison` AS
WITH all_models AS (
  -- H0: Heuristic
  SELECT
    ...
    NULL AS log_loss,
    1 AS sort_order
  FROM `thequantitativeledger.cruzber_models_eu.auc_h0_heuristic`
  ...
  SELECT
    ...
    log_loss,
    2 AS sort_order
  FROM `thequantitativeledger.cruzber_models_eu.eval_h1_logistic`
  ...
  FROM `thequantitativeledger.cruzber_models_eu.eval_oos_h4`
  WHERE TRUE
  LIMIT 1
)
SELECT
  ...
  -- Performance tier
  CASE
    WHEN model_name = 'MAIN_BoostedTree' THEN 'ðŸ¥‡ Reference Model'
    WHEN auc >= 0.85 THEN 'ðŸ¥ˆ Strong Baseline'
    WHEN auc >= 0.70 THEN 'ðŸ¥‰ Reasonable Baseline'
    WHEN auc >= 0.60 THEN 'âš ï¸ Weak Baseline'
...
CREATE OR REPLACE TABLE `thequantitativeledger.cruzber_models_eu.precision_at_k_comparison` AS
WITH all_precision_k AS (
  SELECT model_name, k, precision_at_k, lift_at_k
  FROM `thequantitativeledger.cruzber_models_eu.precision_at_k_h0`
  
  UNION ALL
  
  SELECT model_name, k, precision_at_k, lift_at_k
  FROM `thequantitativeledger.cruzber_models_eu.precision_at_k_h1`
  
  UNION ALL
  
  SELECT model_name, k, precision_at_k, lift_at_k
  FROM `thequantitativeledger.cruzber_models_eu.precision_at_k_h2`
  ...
  SELECT 
    ...
    SAFE_DIVIDE(
      ...
      (SELECT AVG(CAST(y_oos_h4 AS FLOAT64)) FROM `thequantitativeledger.cruzber_models_eu.score_oos_h4_calibrated` WHERE split = 'VAL')
    ) AS lift_at_k
  FROM (
...
CREATE OR REPLACE TABLE `thequantitativeledger.cruzber_models_eu.feature_importance_comparison` AS
WITH boosted_tree_importance AS (
  ...
  FROM `thequantitativeledger.cruzber_models_eu.feature_importance_oos_h4`
  LIMIT 15
),
logistic_importance AS (
  ...
  FROM `thequantitativeledger.cruzber_models_eu.feature_importance_h1`
  LIMIT 15
)
SELECT
  ...
  CASE
    WHEN bt.rank IS NOT NULL AND lr.rank IS NOT NULL THEN 'âœ… Consistent'
    WHEN bt.rank IS NOT NULL THEN 'ðŸŒ³ Tree-only'
    ELSE 'ðŸ“Š Linear-only'
...
FROM `thequantitativeledger.cruzber_models_eu.baselines_comparison`
ORDER BY auc DESC;

-- Output main comparison table
SELECT * FROM `thequantitativeledger.cruzber_models_eu.baselines_comparison` ORDER BY auc DESC;
